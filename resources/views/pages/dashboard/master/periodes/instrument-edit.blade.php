...
@section('content')
	...
	<x-main.section>
		<x-main.card>
			<form action="{{ route('dashboard.master.periodes.instruments.update', [$periode->uuid, $instrument->uuid]) }}" method="POST" class="flex flex-col gap-5">
				@csrf
				@method('PUT')
				<x-form.input name="name" label="Instrumen" placeholder="Isi instrumen" :value="$instrument->name" />
				<x-form.choices name="units[]" label="Area" placeholder="Pilih Area" :multiple="true" :value="$instrument->units
				    ->map(function ($unit) {
