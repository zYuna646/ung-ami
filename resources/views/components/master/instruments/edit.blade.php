<div id="edit-instrument" x-data="{ editInstrumentModal: false }">
	<x-button @click="editInstrumentModal = true" color="success" size="{{ $size ?? 'md' }}">
		Edit
	</x-button>
		...
		<div class="relative w-full max-w-2xl rounded-lg bg-white p-6 shadow-lg">
			...
			<div class="p-4">
				<form action="{{ route('dashboard.master.instruments.update', $instrument->uuid) }}" method="POST" class="flex flex-col gap-5">
					@csrf
					@method('PUT')
					<x-form.input name="instrument" label="Instrumen" placeholder="Isi instrumen" :value="$instrument->instrument" />
					<div class="flex justify-end gap-2">
						<x-button @click="editInstrumentModal = false" color="default">Batal</x-button>
