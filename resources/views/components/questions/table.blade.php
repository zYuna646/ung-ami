		...
		<tbody>
			@foreach ($questions as $question)
				<tr>
					<td>{{ $question->code }}</td>
					<td>{{ $question->text }}</td>
					<td>{{ $question->units->pluck('unit_name')->implode(', ') }}</td>
					<td>
						<div class="flex flex-col gap-2 sm:flex-row">
							<x-button :href="route('dashboard.master.questions.edit', $question->uuid)" color="success" size="sm">
								Edit
							</x-button>
							<x-button.delete :action="route('dashboard.master.questions.destroy', $question->uuid)" color="danger" size="sm">
